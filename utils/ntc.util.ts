...
const ntc = {
  ...
  name(color: string): [string, string, boolean] {
    ...
    let ndf1 = 0; let ndf2 = 0; let ndf = 0;
    let cl = -1; let df = -1;
    ...
    for (let i = 0; i < this.names.length; i++) {
      ...
      ndf1 = Math.pow(r - (this.names[i][2] ?? 0), 2) +
             Math.pow(g - (this.names[i][3] ?? 0), 2) +
             Math.pow(b - (this.names[i][4] ?? 0), 2);

      ndf2 = Math.pow(h - (this.names[i][5] ?? 0), 2) +
             Math.pow(s - (this.names[i][6] ?? 0), 2) +
             Math.pow(l - (this.names[i][7] ?? 0), 2);
