...
export function getRandomColor(): string {
  ...
  const saturation = Math.floor(Math.random() * (70 - 40 + 1)) + 40; // Restrict saturation to 40%-70%
  const lightness = Math.floor(Math.random() * (75 - 50 + 1)) + 50; // Restrict lightness to 50%-75%
